...
M.options = {
   highlights = {
      ...
      GitStashNumber = { fg = "#888888", bold = false },
      GitStashBranch = { fg = "#549afc", bold = true },
      GitStashText = { fg = "#00aa00", bold = false },
      ...
      -- Treesitter & Modern Diff Captures
      ["@added"] = { link = "DiffAdd" },
...
function M.apply_highlights()
   if not M.options or not M.options.highlights then
      return
   end
   for hl_group, spec in pairs(M.options.highlights) do
      vim.api.nvim_set_hl(0, tostring(hl_group), spec)
   end
end
