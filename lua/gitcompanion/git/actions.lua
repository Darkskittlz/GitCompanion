...
-- lua/gitcompanion/git/actions.lua

function M.discard_changes_selected()
   -- Require state module directly to avoid missing global references
   ...
   local sel = Ui.changed_files and Ui.changed_files[Ui.selected_index]
   if not sel then
   ...
   -- Pull git root safely from status module
   ...
   -- Trigger refresh via state module
