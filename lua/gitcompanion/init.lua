...
---------------------------------------------------------------------------
-- Global cache table initialization (ensure this exists in your plugin state)
Ui.diff_cache = Ui.diff_cache or {}
Ui.commit_graph_cache = Ui.commit_graph_cache or {}
Ui.current_branch = Ui.current_branch or ""

-------------------------------------------------------------------------------
-- 1. PURE IN-MEMORY RENDER: LEFT WINDOW
-------------------------------------------------------------------------------
render_left = function()
   ...
   if Ui.mode == "files" or not (Ui.mode == "branches" or Ui.mode == "stashes") then
      if type(render_files_tree) == "function" then
         render_files_tree()
      end
      return
   end
   ...
   if Ui.mode == "branches" then
      local current = Ui.current_branch or ""
...
function fetch_git_graph_async(branch)
   branch = branch or "HEAD"
...
-------------------------------------------------------------------------------
-- 2. PURE IN-MEMORY RENDER: RIGHT WINDOW
-------------------------------------------------------------------------------
render_right = function()
   ...
   local out = Ui.commit_graph_cache[branch]

   -- If cache missing, trigger async fetch and display placeholder
   if not out then
      out = { "[Loading commit graph...]" }
      if type(fetch_git_graph_async) == "function" then
         fetch_git_graph_async(branch)
      end
   elseif #out == 0 then
      out = { "[No commits]" }
   end
   ...
   local graph_chars_list = graph_chars or {}

   for i, line in ipairs(out) do
      -- Highlight graph symbols
      for pos = 1, #line do
...
-------------------------------------------------------------------------------
-- 3. PURE IN-MEMORY RENDER: DIFF WINDOW
-------------------------------------------------------------------------------
render_diff = function()
   if not Ui or not Ui.diff_buf or not vim.api.nvim_buf_is_valid(Ui.diff_buf) then
      return
   ...
   elseif Ui.mode == "branches" then
      if Ui.right_win and vim.api.nvim_win_is_valid(Ui.right_win) then
         ...
         cache_key = line:match("%x%x%x%x%x%x%x+")
...
refresh_ui = function(opts)
   opts = opts or {}

   -- 1. Stop and clear active timer if called within the debounce window
   if refresh_timer then
      vim.fn.timer_stop(refresh_timer)
      refresh_timer = nil
   end

   -- Preserve opts state into the deferred callback
   local skip_fetch = opts.skip_fetch

   -- 2. Debounce by 15ms (1 frame at 60Hz) using vim.fn.timer_start
   refresh_timer = vim.fn.timer_start(
      ...
      vim.schedule_wrap(function()
         ...
         log_sub("State Bounds Clamped")

         -- 2. INSTANT RENDER (Sub-step timing)
         render_left()
         log_sub("render_left Done")

         render_right()
         log_sub("render_right Done")

         render_diff()
         log_sub("render_diff Done")

         -- Update window titles and cursor position
         if Ui.left_win and vim.api.nvim_win_is_valid(Ui.left_win) then
         ...
         log_sub("Window Configs Updated")

         -- 3. BACKGROUND FETCH
         if not skip_fetch then
            local pending_jobs = 0

            local function on_job_complete()
               pending_jobs = pending_jobs - 1
               if pending_jobs <= 0 then
                  local fresh_total = (Ui.mode == "branches") and #(Ui.branches or {})
                      or (Ui.mode == "stashes" and #(Ui.stashes or {}) or #(Ui.changed_files or {}))
                  Ui.selected_index = math.max(1, math.min(Ui.selected_index or 1, math.max(1, fresh_total)))
            ...
            if Ui.mode == "branches" and type(load_branches_async) == "function" then
               pending_jobs = pending_jobs + 1
               load_branches_async(on_job_complete)
            end

            if type(get_changed_files_async) == "function" then
               pending_jobs = pending_jobs + 1
               get_changed_files_async(on_job_complete)
            end
            log_sub("Async Jobs Queued")
         end
...
function M.toggle(opts)
   ...
   -- Preserve or determine initial mode synchronously
   if not Ui.mode then
